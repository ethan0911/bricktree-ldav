...
% and incur frequent CPU-GPU communication to refine the visible data, which hampers the interactivity.
However, these approaches inevitably employ extra data structures (e.g., page tables).
In the extreme case that massive missing data are requested to load during interactive rendering, 
those solutions are likely to incur a CPU-GPU data transfer bottleneck\cite{hadwiger2012interactive}.}
...
% \final{Meanwhile, all these approaches inevitably occur
% some CPU-GPU communications.}
% All these approaches inevitably require
...
The GigaVoxel system \cite{crassin2009gigavoxels} also uses a hierarchical data structure
that is similar to the $N^3$-tree, but with a smaller branching factor of $2^3$ (as shown in
...
\begin{figure}[t]
  ...
  \caption{\label{fig:tree_comparision}Comparison of the data structure with an octree, the GigaVoxel's hierarchy and an $N^3$-tree (Bricktree) in a 2D representation.
  ...
  has a branching factor of $2^3$ and allows for decomposing a node into $N^3$ cells; an $N^3$-tree and a Bricktree share the same data structure but have different layouts. Both have a branching factor of $N^3$ and allow for decomposing a node into $N^3$ cells.}
  \vspace{-1em}
\end{figure}
...
with a 32-bit int, we tile the volume into a ``Bricktree 
Forest'', which consists of a list of Bricktrees, rather than 
encoding the large volume into a single deep Bricktree.
Furthermore, this design not only is conducive to parallel tree traversal on multi-core CPU architectures,
...
rather than by $2^3$. \final{Furthermore, a Bricktree is similar to the $N^3$-tree in terms of  data structure,
but different with respect to data layout in memory. In an $N^3$-tree node, each cell stores either a data
value (for a leaf node) or a reference to the children (for an inner node). In this case, if we expect to store data values for inner nodes to apply progressive rendering, we have to save another index buffer in the large texture and assign an
invalid index (-1) for each cell in a brick when no
cell has children. The number of those indices adds up to $N^3$ for an $N^3$ brick. However, in a Bricktree, we represent bricks with two separate buffers. This choice allows us to save memory 
and store only one invalid index for an $N^3$ brick
if no cell of this brick has children. Hence, a Bricktree is more suitable for progressive 
rendering.}
In the  next section, we will introduce the Bricktree layout in detail.
...
stores a data value. We denote a brick that stores data values as a ``Valuebrick''. Bricks can
...
along with an indexBrickID to correlate the Valuebrick
and Indexbrick.
}
...
this value is the user's choice. In our implementation, we compute this value by averaging
...
In \Cref{fig:bricktree_layout}, a Valuebrick is shown in purple and an Indexbrick in light blue. 
}
...
it does not refer to an Indexbrick and thus no cell in the brick has children;
...
1) one high-level XML file represents the Bricktree Forest, which contains meta-data ($N,T,M,L,K$ etc.) of the input volume; 
...
to the number of leaf bricks.
% (see  \Cref{fig:loadbylevel}).
...
in the tree. \final{This holds in practice - the overhead of Bricktree with $N=4$ is 8.0\% on
the magnetic reconnection dataset and the DNS dataset. The details of the benchmarks can be found in \Cref{sec:results}.}
By contrast, the overhead of an octree (Bricktree with $N=2$) that is built on top of those data is 43\% 
and 72\%, respectively. 
}
...
into multi-resolution bricks is mostly ignored in the volume rendering literature\cite{fogal2013analysis}. However, performance becomes a significant
...
our ``\textit{ospRawToBricks}'' tool employs the \textit{GNU make} command to meet this goal.
This choice allows us to specify a customized number of trees to build in parallel in case we 
consume all the host memory. 
...
A simple but inefficient approach is a serial implementation with the C/C++ code without
maintaining a copy of the Bricktree structure on the ISPC side. We can easily 
implement a C/C++ version of the sample function \final{(see \Cref{alg:sample})} that has direct
...
When our data exceed 4~GB, 32-bit addressing will fail in \final{\Cref{alg:sample}},
...
the performance when running our application in the ISPC 64-bit addressing mode.}
...
\final{An interactive and smooth visualization is achieved in coordination with the progressive rendering.
}
...
\item The DNS, produced by the Institute for Computational Engineering and Science (ICES) at the
	University of Texas-Austin, is a single $10240 \times 7680 \times 1536$ double precision volume (size: 900 GB) from a turbulent flow simulation. \final{In our experiments, we use a float version of the DNS data (450 GB)}.  
\end{itemize}
...
We would expect each sample to be several times as costly as for  
the reference renderer. However, given our access-friendly data layout and ISPC-optimized tree
traversal, we see in \Cref{fig:exp_ospray_bricktree} that the rendering performance for fully loaded 
data is only about $35\%$ slower. \final{Despite some performance loss, 
\Cref{fig:exp_ospray_bricktree_framerate} indicates that our Bricktree module still runs at interactive framerates. The benchmarks are run from a fixed viewpoint with the volume covers through 3/4 of the screen
and with a semi-transparent transfer function. Images of the benchmarks are provided in the supplementary
material.}
% The details of camera setting and transfer function can be found in the supplementary material.}
...
% \begin{figure}[h]
%     \centering
%         \includegraphics[width=0.9\columnwidth]{overall_performance.png}
%         \label{fig:overall_performance}
% 	\caption{\label{fig:overall_performance} \final{Framerate of rendering four datasets on FSM with three viewpoints. Viewpoint 1: a far perspective camera; viewpoint 2: a near perspective camera; viewpoint 3: a near perspective camera with a very transparent transfer function.}}
% 	\vspace{-1em}
% \end{figure}
...
\Cref{table:brick_size} demonstrates that, for the DNS dataset, the best rendering 
...
compression threshold affects the final image quality should be performed. 
On the other hand, a hierarchical structure naturally leads to adaptive sampling. 
It would be interesting to further apply the adaptive sampling to our Bricktree module. 
Eventually, we would like to integrate our Bricktree into general-purpose visualization frameworks. 
\final{We are also interested in testing the Bricktree structure in GPUs architecture}. 
}
...
\acknowledgments{
This project was supported in part by the Intel Graphics and Visualization Institutes of XeLLENCE and the National Institute of General Medical Sciences of the National Institutes of Health under grant number P41 GM103545-18. The authors wish to thank Paul Navr\'atil from TACC for the use of DNS datasets and the Lawrence Livermore National Laboratory for the use of Richtmyer-Meshkov datasets. We would also like to thank Nathan Marshak for reviewing and polishing the manuscript.}
