...
\begin{figure}[h]
  ...
  In a 3D scenario, an octree has a branching factor of $2^3$ and allows for decomposing a node into $2^3$ cells; A $N^3$-tree has a branching factor
...
cube ($M \neq L \neq K $).
A Bricktree is a generalization of an octree where we subdivide a node by an arbitrary number
...
Also along with a indexBrickID to indicate the corresponding relationship between the Valuebrick
and the Indexbrick.
}

\feng{
Each cell of a Valuebrick corresponds to a set of voxels in the volume, For a ``leaf brick''
(e.g. $N_{16}$), one cell corresponds to exactly one voxel and the cell's value equals to the
value of this voxel. By contrast, each cell of an ``inner brick'' (e.g. $N_0$) typically
represents a set of voxels. Accordingly, we can assign a value to each cell by averaging
over all voxels in this set. Each cell of an Indexbrick refers to a child brick the current
brick has children. For a ``leaf brick'', it has no children and thus no Indexbrick. In this
case, the indexBrickID of this brick is set to invalid (-1). While for an ``inner brick'', its
...
volume with dimension of $M \times L \times K$, which is organized into a BrickTree Forest.
...
while other children are inner nodes. Recall that for each inner node, we need to store 
both a Valuebrick and an Indexbrick. 
This will result in more memory consumption than in the ideal case.
However, the overhead will still most likely be considerablely less than that of an octree. 
...
(see \Cref{table:brick_size}). By contras, the overhead of octree (Bricktree with $N=2$) is 43\% 
and 72\% respectively. 
}
...
It is straightforward that we can construct the Bricktree Forest in parallel. The ``\textit{ospRawToBricks}''
...
See \Cref{alg:bricktree} for the algorithm used to recursively construct a Bricktree. 
...
Furthermore, we allow the user to set a threshold ($t$) that determine which input regions
...
the performance when we recompile our application in 64-bit.}
...
which may results in measured bandwidt value that appear to exceed the underlying
...
Despite some performane loss, \Cref{fig:exp_ospray_bricktree_framerate}
...
performance since it only needs to
load metadata prior to rendering. With both renderers, loading time increases with
...
By contrast, with our Bricktree module, the DNS dataset loads in under 40 seconds.
