...
but different with respect to data layout in memory. Each cell in an $N^3$-tree node stores either a data
value (for a leaf node) or a reference to the children (for an inner node). If we expect to apply progressive rendering and store data values for inner nodes in Lefebvre's $N^3$-tree \cite{lefebvre2005octree}, we have to save another index buffer in the large texture and assign an
invalid index (-1) for each cell in a brick when no
cell has children. The number of those indices adds up to $N^3$ for an $N^3$ brick. In a Bricktree, we represent bricks with two separate buffers. This choice allows us to save memory 
and store only one invalid index for $N^3$ cells in a brick
if no cell has children. 
