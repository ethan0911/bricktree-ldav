...
memory. However, the implementation has some performance penalties, because 64-bit addressing is slightly more costly than 32-bit addressing. In an ideal case, memory accesses are mostly coherent, and only a few iterations of the aforementioned loop are necessary. \feng{In the worse case that a lot of iterations are needed, there may be performance penalties. However, the performance will be preferable to the worst case
...
\feng{All datasets are stored on an Isilon file server and loaded through a 1 GB ethernet connection. 
The I/O throughput of the file system is 1.6 GB/s and 3.5 GB/s. }
\begin{itemize}
