...
that is similar to the $N^3$-tree, but has a smaller branching factor of $2^3$.
...
\begin{figure}[h]
  ...
  \caption{\label{fig:tree_comparision}Comparison of an octree, an $N^3$-tree and a Bricktree (2D representation).
...
By encoding a sub-volume into a Bricktree, we guarantee that using a 32-bit int is
enough to index all the bricks rather than a 64-bit int. 
